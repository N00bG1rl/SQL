...
/* 9.4 The query shown gives the number of routes that visit either London Road (149) or Craiglockhart (53).
...
/* 9.5 Execute the self join shown and observe that b.stop gives all the places you can get to from Craiglockhart, without changing routes.
Change the query so that it shows the services from Craiglockhart to London Road. */
SELECT a.company, a.num, a.stop, b.stop FROM route a
    JOIN route b ON (a.company=b.company AND a.num=b.num)
WHERE a.stop=53 AND b.stop=149

/* 9.6 The query shown is similar to the previous one, however by joining two copies of the stops table we can refer to
...
/* 9.7 Give a list of all the services which connect stops 115 and 137 ('Haymarket' and 'Leith'). */
SELECT DISTINCT a.company, a.num FROM route a
    JOIN route b ON (a.company=b.company AND a.num=b.num)
WHERE a.stop=115 AND b.stop=137

/* 9.8 Give a list of the services which connect the stops 'Craiglockhart' and 'Tollcross'. */
...
/* 9.9 Give a distinct list of the stops which may be reached from 'Craiglockhart' by taking one bus,
...
/* 9.10 Find the routes involving two buses that can go from Craiglockhart to Lochend.
...
/* Your Quizz score is: 3 out of 3. */